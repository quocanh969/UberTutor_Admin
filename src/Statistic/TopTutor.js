...
export default class TopTutor extends Component {
    constructor(props) {
        super(props);
        this.state = {
            method: 0,
    ...
    statistic() {
        switch(this.state.method)
        {
            case 0:
            {
                contractServ.getTutorByIncomeFromLastNDays(1)
                .then(res=>{
                    if(res.code === 1)
                    {
            ...
            case 1:
            {
                contractServ.getTutorByIncomeFromLastNDays(7)
                .then(res=>{
                    if(res.code === 1)
                    {
                        this.setState({topList: res.info.data});
            ...
            case 2:
            {
                contractServ.getTutorByIncomeFromLastNDays(30)
                .then(res=>{
                    if(res.code === 1)
                    {
                        this.setState({topList: res.info.data});
            ...
            case 3:
            {
                contractServ.getTutorByIncomeFromLastNDays(90)
                .then(res=>{
                    if(res.code === 1)
                    {
                        this.setState({topList: res.info.data});
            ...
            case 4:
            {
                contractServ.getTopTutorsAllTime()
                .then(res=>{
                    if(res.code === 1)
                    {
                        this.setState({topList: res.info.data});
    ...
    render() {
        return (
            <div className='container-fluid mt-3'>
                ...
                <div>
                {this.state.method === 0
                ?'Top 5 tutor have most income of 1 day'
                :(this.state.method === 1
                ?'Top 5 tutor have most income of 7 days'
                :(this.state.method === 2
                    ?'Top 5 tutor have most income of 30 days'
                    :(this.state.method === 3
                        ?'Top 5 tutor have most income of 90 days'
                        :'Top 5 tutor have most income of all times')))}
                </div>
